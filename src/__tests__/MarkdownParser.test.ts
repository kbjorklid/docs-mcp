...
This is the main content.`;
      ...
      expect(mockYaml.load).toHaveBeenCalledWith("title: Test Document\ndescription: A test document\nkeywords: [test, document]");
...
# Content`;
      ...
      expect(consoleSpy).toHaveBeenCalledWith("Error parsing YAML front matter:", expect.any(Error));
      expect(result).toEqual({
        metadata: {},
        content: `---
invalid: yaml: content:
  ...
  describe("generateSectionId", () => {
    it("should generate valid section IDs", () => {
      const testCases = [
        ...
        { input: "Mixed-CASE_and_underscores", expected: "mixed-case_and_underscores" },
        { input: "", expected: "" },
...
More content.`;
      ...
      const subtitleSection = result.sections.find(s => s.id === "title/subtitle");
...
Content 1`;
      ...
      const { sectionMap } = MarkdownParser.parseMarkdownSections(content);
      const result = MarkdownParser.readSectionsFromContent(content, [], sectionMap);
...
Advanced content here.`;
      ...
      // Read file
      const { content, metadata: parsedMetadata } = MarkdownParser.readMarkdownFile(filePath);
